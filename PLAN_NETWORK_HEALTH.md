...
- No test target and no tests. The package currently has neither; do not add them. Correctness is enforced through the design constraints in this document (structural seam placement, centralized classification, once-only recording guards) and a clean strict-concurrency build.
...
16. **`AppException` catalog**: networking error identities are catalogued as 4-hex-digit `AppException` constants compared via `exception.isEqual(to:)` (`Sources/Modules/Common/Extensions/Public/AppException+CommonExtensions.swift`). `Exception.timedOut` and `.internetConnectionOffline` come from AppSubsystem; their `AppException` identities (if needed) live there.
...
- **Time source:** use wall-clock time (`Date.now`) captured inside the actor for all decay math; keep every time read in one place in the estimator so the math stays coherent.
- **Path monitor:** first check whether AppSubsystem's `Build.isOnline` implementation exposes an observable `NWPathMonitor`/path publisher you can reuse for transition events and `isConstrained`/`isExpensive`. If it only exposes the Bool, add an internal `NWPathMonitor` owned by the health service with an explicit lifecycle: started once for the shared service, with a `stop()`/teardown path so monitors and their queues are never leaked.
...
- **Resolution semantics**, centralized in one internal pure function (health snapshot + configuration in, concrete strategy out) so the mapping cannot drift between the two funnels:
...
- [ ] No test target added; the package continues to contain only `Sources/`.
- [ ] README and DocC updated.

## 11. Verify Against AppSubsystem Before Coding
...
3. **`Exception` underlying-error introspection**: only relevant if extending the classifier to transport failures (§5.1); default is not to.
4. **`DevModeService.insertAction` index semantics** beyond the observed usage (insert at 2 after the two existing actions).

Document each resolution in the implementation summary you produce, alongside the deviations already called out: storage timeouts feed nothing (§5.1), `noValueExists` counts as success latency (§5.1), latency anchoring via shared constant instead of nonexistent timeout config (§5.3), recording surface on the delegate protocol (§6), and enum case over resolver (§7).
