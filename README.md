...
Use `observe(at:)` to receive a stream of values as they change at a given path. The method returns an `AsyncThrowingStream` that emits the current value immediately and again each time it changes on the server:
